...
Sub-commands (run via `python -m analysis.rq2.statistical <command>`):
  ...
  multi-baseline-table   same table against CART/J48/REPTree at once (add -combine to merge datasets)
  holm-table             cross-baseline Holm-Bonferroni correction on top of multi-baseline-table's
                         combined output -- this is the final paper table
"""
